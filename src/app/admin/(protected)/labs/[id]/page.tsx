...
async function updateLabAction(id: string, formData: FormData): Promise<void> {
  ...
  if (!validated.success) {
    redirectWithErrors(`/admin/labs/${id}`, validated.errors);
  }

  enforcePublishEligibility(publishRequested, `/admin/labs/${id}`, validated.value);

  const mdxError = await validateMdxBody(validated.value.body);
  if (mdxError) {
    redirectWithErrors(`/admin/labs/${id}`, { body: mdxError });
  }

  if (await isLabSlugTaken(validated.value.slug, id)) {
    redirectWithErrors(`/admin/labs/${id}`, { slug: "Slug must be unique." });
  }
