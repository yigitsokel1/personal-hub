...
import { DeleteProjectButton } from "@/app/admin/(protected)/projects/delete-project-button";
import { listAdminProjects } from "@/lib/content-source/get-projects";
  ...
  return (
    <main>
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-black/60">
            Manage projects from the database-backed admin flow.
          </p>
        </div>
        <Link
      ...
      <div className="mt-3 min-h-5 text-sm">
        {params.status === "saved" ? <p className="text-green-700">Project saved.</p> : null}
        {params.status === "deleted" ? <p className="text-green-700">Project deleted.</p> : null}
      ...
      {items.length === 0 ? (
        <p className="mt-8 text-sm text-black/55">No project entries yet.</p>
      ) : (
